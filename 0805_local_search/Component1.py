...
class Painter:
    ...
    def draw(self, problem : Problem, hub_nm):
        ...
        for car in problem.cars:
            ...
            #four_lat_coords = [left_coord[0], right_coord[0], down_coord[0], up_coord[0]]
            #four_lon_coords = [left_coord[1], right_coord[1], down_coord[1], up_coord[1]]
        ...
        not_dup_lat_list = [x for x in latitude_list if x not in gg_four_lat_coords]
        not_dup_lon_list = [x for x in longitude_list if x not in gg_four_lon_coords]
        ...
        for i in range(len(dup_lat_index)):
            ...
            for j in range(len(latitude_list)):
                for k in range(len(gg_four_lat_coords)):                                
                    if latitude_list[j] not in gg_four_lat_coords and longitude_list[j] not in gg_four_lon_coords:
                        if to_find_coord_index == 0 and left_coord_ok == False:
                            #left_coord
                            temp_lat = min(not_dup_lat_list)
                            temp_lon = longitude_list[latitude_list.index(min(not_dup_lat_list))]
                            ...
                            temp_lon = longitude_list[latitude_list.index(max(not_dup_lat_list))]
                            ...
                            temp_lat = latitude_list[longitude_list.index(max(not_dup_lon_list))]
                            ...
                            temp_lat = latitude_list[longitude_list.index(min(not_dup_lon_list))]
        ...
        """
        for j in range(len(gg_four_lat_coords)):
            four_lat_coords.append(gg_four_lat_coords[j])
            four_lon_coords.append(gg_four_lon_coords[j])
        """
        #방향구분을 해주기 위해서
        #왼쪽: 빨간색, 오른쪽: 파란색, 위쪽: 노란색, 아래쪽: 초록색
        gmap3.scatter(gg_four_lat_coords[:0], gg_four_lon_coords[:0], 'red', size=1000, marker=False)
        gmap3.scatter([gg_four_lat_coords[1]], [gg_four_lon_coords[1]], 'blue', size=1000, marker=False)
        gmap3.scatter([gg_four_lat_coords[2]], [gg_four_lon_coords[2]], 'green', size=1000, marker=False)
        gmap3.scatter([gg_four_lat_coords[3]], [gg_four_lon_coords[3]], 'yellow', size=1000, marker=False)

        xxx = sum(gg_four_lat_coords)/len(gg_four_lat_coords)
        yyy = sum(gg_four_lon_coords)/len(gg_four_lon_coords)
        x1 = gg_four_lat_coords[0]
        x2 = gg_four_lat_coords[1]
        x3 = gg_four_lat_coords[2]
        x4 = gg_four_lat_coords[3]
        y1 = gg_four_lon_coords[0]
        y2 = gg_four_lon_coords[1]
        y3 = gg_four_lon_coords[2]
        y4 = gg_four_lon_coords[3]
        #print(x1,x2,x3,x4,y1,y2,y3,y4)
        #print((3*(y2-y4)*(x1-x3)-3*(y1-y3)*(x2-x4)))
        #print((3*(x1-x3)*(y2-y4)-3*(x2-x4)*(y1-y3)))
        #center_x = ((x1-x3)*((y2-y4)*(x1+x3) + y2*x2-y4*x4)-(x2-x4)*((y1-y3)*(x2+x4)+x1*y1-y3*x3))/(3*(y2-y4)*(x1-x3)-3*(y1-y3)*(x2-x4))
        #center_y = ((y2-y4)*((x1-x3)*(y2+y4)+x1*y1-x3*y3)-(y1-y3)*((x2-x4)*(y1+y3)+(x2*y2)-(x4*y4)))/(3*(x1-x3)*(y2-y4)-3*(x2-x4)*(y1-y3))
        #노란점은 네 개의 점 좌표들의 평균
        ...
        #four_coord = zip(*[(gg_four_lat_coords[0], gg_four_lon_coords[0]),(gg_four_lat_coords[1], gg_four_lon_coords[1]),(gg_four_lat_coords[2], gg_four_lon_coords[2]),(gg_four_lat_coords[3], gg_four_lon_coords[3])])
        lat = []
        lon = []
        ...
        """
        if intersect_line([gg_four_lat_coords[3], gg_four_lon_coords[3]],[gg_four_lat_coords[2], gg_four_lon_coords[2]],[gg_four_lat_coords[0], gg_four_lon_coords[0]], [gg_four_lat_coords[1], gg_four_lon_coords[1]]) == True:
            print("교차함")
        else:
            print("교차하지 않음.")
        """
        ...
        print("으하하하하", left_coord_start_candidate)
        ...
        polygon1 = Polygon(path_list)
        fig, ax = plt.subplots()
        ax.invert_xaxis()
        ax.invert_yaxis()
        a1,b1 = polygon1.exterior.xy
        ax.plot(a1,b1)
        plt.show()

        gmap3.polygon(real_lat, real_lon, face_color='pink') 
        #4등분 한 뒤 폴리곤
        #만약 모든 좌표들의 평균을 사용한다면?
        #총 4개의 삼각형이 나와야함

        #up_tri = 

        #gmap3.polygon([x1,x2,center_latitude], [y1,y2,center_longitude], face_color='pink')
        #gmap3.polygon([x2,x3,center_latitude], [y2,y3,center_longitude], face_color='blue')
        #gmap3.polygon([x3,x4,center_latitude], [y3,y4,center_longitude], face_color='green')
        #gmap3.polygon([x4,x1,center_latitude], [y4,y1,center_longitude], face_color='yellow')
        #cv2.imshow('final', map_for_all)
